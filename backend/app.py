...
def read_root():
    return {
        "message": "Welcome to the AI-Powered Supply Chain Dashboard Backend"
    }
...
def predict_supplier(
    ...
    return {
        "predicted_reliability": int(prediction[0])
    }
...
def predict_inventory():
    prediction = inventory_model.forecast(steps=7)

    return {
        "inventory_forecast": prediction.tolist()
    }
...
def predict_shipment(delay_time: float):
    ...
    return {
        "predicted_delay": bool(prediction[0])
    }
...
class SupplyChainReport(FPDF):
    ...
    def header(self):
        ...
        self.cell(
            0,
            10,
            "Supply Chain Optimization Dashboard Report",
            ln=True,
            align="C"
        )
        self.ln(10)
...
def build_report():
    ...
    pdf.multi_cell(
        ...
        10,
        (
            "This report summarizes key performance metrics, "
            "predictive insights, and supply chain optimization "
            "data for the observed period. It combines AI-powered "
            "forecasts with operational data to assist decision-makers "
    ...
    pdf.add_metric(
        "Predicted Inventory Demand (Next 7 Days)",
        "450, 460, 475, 480, 490, 500, 520"
    )

    pdf.add_metric(
        "Potential Shipment Delays",
        "Shanghai -> Los Angeles (High risk of delay)"
    )

    pdf.add_metric(
        "Supplier Risk Alert",
        "Global Parts Ltd (88.0% reliability) flagged for review."
    )

    pdf.ln(10)

    # Save PDF
    output_path = "SupplyChainReport.pdf"
...
def generate_report():
    ...
    return FileResponse(
        ...
        filename="SupplyChainReport.pdf"
    )
